  ...
  protected void doGet(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
    ...
    try (Connection c = DBUtil.getConnection()) {
      PreparedStatement ps = c.prepareStatement(
        "SELECT p.patient_id,u.name,p.age,p.gender FROM patients p JOIN users u ON p.user_id=u.user_id WHERE u.email=?");
      ...
      req.setAttribute("doctors", c.createStatement().executeQuery(
        "SELECT d.doctor_id,u.name AS dname,d.specialization FROM doctors d JOIN users u ON d.user_id=u.user_id"));
